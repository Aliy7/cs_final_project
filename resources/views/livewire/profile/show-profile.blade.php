    ...
    <div class="container mx-auto max-w-4xl p-5 bg-white rounded-lg shadow-lg">
        @if ($profile && $profile->user)
            ...
            <div class="mb-3">
                ...
                {{ $profile->date_of_birth ? \Carbon\Carbon::parse($profile->date_of_birth)->format('Y-m-d') : 'N/A' }}
            </div>

            <div class="mb-3">
                <strong>Image:</strong>
                @if ($profile->image_url)
                    <div><img src="{{ $profile->image_url }}" alt="Profile Image" class="w-24 h-24 rounded-lg shadow">
                    </div>
                @else
                    <div
                        class="w-24 h-24 overflow-hidden bg-gray-100 rounded-full dark:bg-gray-600 flex items-center justify-center">
