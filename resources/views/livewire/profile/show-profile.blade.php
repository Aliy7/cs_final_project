    ...
    <div class="container mx-auto max-w-4xl p-5 bg-white rounded-lg shadow-lg">
        @if($profile->user)
            ...
            <div class="mb-3">
                ...
                <strong>Date of Birth:</strong> {{ $profile->date_of_birth instanceof \Carbon\Carbon ? $profile->date_of_birth->format('Y-m-d') : 'N/A' }}
            </div>

            <div class="mb-3">
                <strong>Image:</strong>
                @if($profile->image_url && @getimagesize($profile->image_url))
                    <div><img src="{{ $profile->image_url }}" alt="Profile Image" class="max-w-xs rounded-lg shadow"></div>
                @else
