        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-6">
            <div class="p-6 text-gray-900 dark:text-gray-100">
                ...
                @if ($foodListings && $foodListings->count() > 0)
                            ...
                            <div class="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg flex flex-col items-center">
                                <div class="w-full flex justify-between items-center">
                                    <div class="flex flex-col items-start">
                                        <span class="font-medium">Listed by: <a href="#"
                                ...
                                <div x-data="{ seeMore: false }">
                                    <button @click="seeMore = !seeMore"
                                        class="mt-2 text-blue-500 hover:text-blue-700 cursor-pointer">
                                        See More
                                    </button>
                                    <div x-show="seeMore" x-cloak>
                                        ...
                                        <div>Status:
                                            <span
                                                :class="{
                                                    'bg-green-100 text-green-800': {{ $listing->status }},
                                ...
                                <div x-data="{ open: false }">
                                    <button @click="open = !open" class="focus:outline-none">
                                        <img src="{{ asset('storage/logo/icons8-location (1).gif') }}"
                                            alt="Show Location" />
                                    </button>
                                    <div x-show="open" x-cloak>
                                        @if ($listing->location && $listing->location->latitude && $listing->location->longitude)
                                            <div id="map-{{ $listing->id }}" class="map-container w-full h-56"
                                                x-init-map="open" data-listing-id="{{ $listing->id }}"
                    ...
                    <div class="mt-5 text-black font-bold bg-gray-200 p-4">
                        {{ $foodListings->links() }}
                    </div>
                @else
