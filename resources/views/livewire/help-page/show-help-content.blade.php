<div class="profile-content">
    <section class="bg-white dark:bg-gray-900">
        <div class="py-8 px-4 mx-auto max-w-screen-xl sm:py-16 lg:px-6">
            <h2 class="mb-8 text-4xl tracking-tight font-extrabold text-center text-gray-900 dark:text-white">Frequently
                Asked Questions</h2>
            <div class="text-left border-t border-gray-200 dark:border-gray-700">
                @foreach ($FAQContents as $content)
                    <div class="border-b-4 border-blue-500 py-6">
                        ...
                        @if (!$editingContent || $editId != $content->id)
                            ...
                            <div class="flex justify-between items-center">
                                <span class="text-sm text-gray-600 dark:text-gray-400">
                                    Posted by {{ optional($content->user)->username ?? 'Unknown' }} on
                        ...
                        @if ($editingContent && $editId == $content->id)
                            <div>
                                <input wire:model.defer="editTitle" class="form-input mt-2 block w-full"
                                    placeholder="Title">
                                <textarea wire:model.defer="editContent" class="form-textarea mt-1 block w-full" rows="4"
                                    placeholder="Description"></textarea>
                                <input wire:model.defer="editCategory" class="form-input mt-2 block w-full"
