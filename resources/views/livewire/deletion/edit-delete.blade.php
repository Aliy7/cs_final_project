...
<div>
    ...
    <div class="relative">
        ...
        <!-- Dropdown Menu -->
         <!-- Dropdown Menu -->
    <div id="dropdownMenu-{{ $foodListingId }}" style="display: none;" class="absolute right-0 mt-2 bg-blue-500 text-white font-bold py-1 px-1 overflow-hidden">
    ...
    <div id="editModal" class="fixed inset-0  top-200 overflow-y-auto flex items-center justify-center z-10">
        <div class="flex items-center justify-right min-h-screen">
            ...
            <div class="bg-white rounded-lg overflow-hidden transform transition-all sm:w-full sm:max-w-4xl lg:max-w-6xl">
                    ...
                    <div class="space-y-4">
                        <div class="space-y-4">
                            ...
                            <div class="mb-4">
                                ...
                                @error('ingredients') <span class="error text-red-500 text-xs">{{ $message }}</span> @enderror
                            </div>
                        
                            <div class="mb-4">
                                <label for="quantity" class="block text-sm font-medium text-gray-700">Quantity</label>
                                <input type="number" id="quantity" wire:model.defer="quantity" min="1" class="form-input mt-1 block w-full" />
                                @error('quantity') <span class="error text-red-500 text-xs">{{ $message }}</span> @enderror
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                @error('allergen') <span class="error text-red-500 text-xs">{{ $message }}</span> @enderror
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                @error('description') <span class="error text-red-500 text-xs">{{ $message }}</span> @enderror
                            </div>
                        
                            <div class="mb-4">
                                <label for="status" class="block text-sm font-medium text-gray-700">Status</label>
                                <input type="checkbox" id="status" wire:model.defer="status" class="form-checkbox h-5 w-5 text-blue-600" value="1"><span class="ml-2 text-gray-700">Available</span>
                                @error('status') <span class="error text-red-500 text-xs">{{ $message }}</span> @enderror
                            </div>
                        
                            <div class="mb-4">
                                <label for="category_id" class="block text-sm font-medium text-gray-700">Category</label>
                                <select id="category_id" wire:model.defer="category_id" class="form-select mt-1 block w-full">
                                    <option value="">Select a Category</option>
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}">{{ $category->name }}</option>
                                    @endforeach
                                </select>
                                @error('category_id') <span class="error text-red-500 text-xs">{{ $message }}</span> @enderror
                            </div>
...
<script>
    ...
    if (dropdown) {
        ...
        // If the dropdown is null, log an error to the console for debugging
